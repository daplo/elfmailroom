...
export function createLetterCheckout(stripe,{email,orderId,origin,returnUrl,language='en'}){
 const code=localeCode(language),t=key=>translate(code,key);
 return stripe.checkout.sessions.create({
  mode:'payment',customer_email:email,locale:code,
  line_items:[{quantity:1,price_data:{currency:santaLetterPrice.currency,unit_amount:santaLetterPrice.amount,product_data:{name:t('Personalised Santa Letter'),description:t(digitalNotice)}}}],
