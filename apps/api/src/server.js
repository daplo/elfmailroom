...
function ownedOrder(req){
 const order=db.prepare('SELECT * FROM orders WHERE id=?').get(req.params.id);if(!order)return null;
...
app.get('/api/orders/:id',async(req,res)=>{
 ...
 if(order.status!=='paid'&&stripe&&order.checkout_id){try{const checkout=await stripe.checkout.sessions.retrieve(order.checkout_id);if(checkout.payment_status==='paid'){db.prepare("UPDATE orders SET status='paid' WHERE id=?").run(order.id);order.status='paid';}}catch{}}
