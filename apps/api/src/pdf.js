...
const script=fileURLToPath(new URL('../assets/fonts/Caveat.ttf',import.meta.url));
 ...
 return new Promise((resolve,reject)=>{
  ...
  try{
   ...
   function stationery(){
    ...
    doc.x=58;doc.y=170;doc.font('Letter').fontSize(design==='jolly'?10.5:11).fillColor(theme.ink);
   }
   stationery();doc.on('pageAdded',stationery);
   const {greeting,body}=splitLetterGreeting(letter,language);
   if(greeting){
    doc.font('Signature').fontSize(26).fillColor(theme.accent).text(greeting,58,170,{width:doc.page.width-116,lineGap:0,paragraphGap:0});
    doc.y+=10;
   }
   doc.font('Letter').fontSize(design==='jolly'?10.5:11).fillColor(theme.ink);
   ...
   doc.font('Signature').fontSize(28).fillColor(theme.accent).text(copy.signature||copy.santa,58,signY,{lineBreak:false});
