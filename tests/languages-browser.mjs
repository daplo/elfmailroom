...
try{
 for(const {code} of languages){
  ...
  await page.goto(`${baseURL}/write/purchase/${id}#token=${token}`);await page.getByRole('button',{name:t('Request changes'),exact:true}).click();await page.getByLabel(t('What would you like Santa to change?'),{exact:true}).waitFor();assert.equal(await page.locator('.raster-preview').getAttribute('lang'),code);await page.locator('.raster-preview:not([inert] *)>img').waitFor();assert.equal(await page.locator('.preview-accessible p').textContent(),localizedSample(code,'Zoë'));await page.getByRole('button',{name:t('Accept & download PDF'),exact:true}).waitFor();assert(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),`${code} purchase overflow`);
  await context.close();
 }
