...
try{
 for(const {code} of languages){
  ...
  await page.locator('.raster-preview>img').waitFor();assert((await page.locator('.raster-preview>img').getAttribute('src')).startsWith('data:image/jpeg;base64,'));
  ...
  await page.goto(baseURL+languagePath(code));await page.locator('.raster-preview>img').waitFor();assert(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),`${code} landing overflow`);await page.screenshot({path:`tests/landing-${code}-mobile.png`,fullPage:true});const id='aaaaaaaa-aaaa-4aaa-aaaa-aaaaaaaaaaaa',token='a'.repeat(64);
  await page.route('**/api/orders/**',route=>route.fulfill({json:{id,status:'paid',design:'jolly',language:code,generationStatus:'ready',emailStatus:'not_requested',version:1,accepted:false,rewritesRemaining:5,canRewrite:true,letter:localizedSample(code,'Zoë')}}));
  await page.goto(`${baseURL}/write/purchase/${id}#token=${token}`);await page.getByRole('button',{name:t('Request changes'),exact:true}).click();await page.getByLabel(t('What would you like Santa to change?'),{exact:true}).waitFor();assert.equal(await page.locator('.raster-preview').getAttribute('lang'),code);await page.locator('.raster-preview>img').waitFor();assert.equal(await page.locator('.preview-accessible p').textContent(),localizedSample(code,'Zoë'));await page.getByRole('button',{name:t('Accept & download PDF'),exact:true}).waitFor();assert(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),`${code} purchase overflow`);
  await context.close();
 }
