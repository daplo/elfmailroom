...
- Generated `sitemap.xml` containing only the five public pages, with matching language alternates; generated `robots.txt` points to it.
...
The internal admin interface remains in English. SEO metadata targets the five public customer landing pages, not private purchase URLs.
...
The landing build runs Vite, then `apps/landing/scripts/prerender.mjs`. It writes `/index.html`, `/de/index.html`, `/es/index.html`, `/fr/index.html`, `/pl/index.html`, `/robots.txt` and `/sitemap.xml` to `apps/landing/dist`. `npm start` serves them through Express. Rebuild when changing public copy, translations or the canonical origin. Static hosting should serve these real files and return 404 for unknown public paths.
...
The SEO browser test uses the production Express server and generated files. It checks all five pages with JavaScript disabled, hydration without errors, URL/metadata changes, live offer gating, responsive headings, schema uniqueness, sitemap and private-page indexing instructions. These are local checks, not a claim of Google indexing or a passed live Rich Results Test.

After deployment, verify the real domain in Search Console, submit `/sitemap.xml`, inspect the five language URLs, and run Google's Rich Results Test on the live pages. Search appearance is decided by Google; valid structured data does not guarantee a rich result. Complete the real contact, privacy and purchase policies before opening sales. This change does not invent those policies or claim they exist.
