...
import {getDesign} from './config.js';
...
export async function renderPreview({text,design,language,width}){
 ...
 // Measure and paint only after the actual embedded body font is available.
 const [letterFonts,scriptFonts]=await Promise.all([document.fonts.load('400 16px ElfLetter',text+copy.santa),document.fonts.load('600 32px ElfSignature',text+(copy.signature||copy.santa))]);
 if(!letterFonts.length||!scriptFonts.length)throw new Error('Letter font unavailable');
 await document.fonts.ready;
 const art=new Image();art.src=`/assets/stationery/${theme.art}.webp`;await art.decode();
 const canvas=document.createElement('canvas'),ctx=canvas.getContext('2d');
 const margin=width<400?28:45,font=width<400?14:16,lineHeight=font*1.65;
 const {greeting,body}=splitLetterGreeting(text,language);
 const greetingFont=width<400?28:32,greetingLineHeight=greetingFont*1.15;
 ctx.font=`600 ${greetingFont}px ElfSignature`;
 ...
 ctx.font=`400 ${font}px ElfLetter`;const lines=wrap(ctx,body,width-margin*2);
 ...
 ctx.font=`400 ${width<400?30:36}px ElfLetter`;ctx.fillStyle=theme.ink;ctx.fillText(copy.santa,width/2,top-48,width-margin*2);
 ...
 if(design==='jolly'){ctx.save();ctx.lineWidth=1.5;ctx.setLineDash([8,8]);for(const x of [width*.017,width*.983]){ctx.strokeStyle='#438453';ctx.beginPath();ctx.moveTo(x,width*.31);ctx.lineTo(x,height-footer);ctx.stroke()}ctx.restore()}
 ctx.textAlign='left';ctx.fillStyle=theme.accent;ctx.font=`600 ${greetingFont}px ElfSignature`;
 greetingLines.forEach((line,i)=>ctx.fillText(line,margin,top+greetingFont+i*greetingLineHeight));
 ctx.fillStyle=theme.ink;ctx.font=`400 ${font}px ElfLetter`;lines.forEach((line,i)=>ctx.fillText(line,margin,bodyTop+22+i*lineHeight));
 const signY=bodyTop+lines.length*lineHeight+38;ctx.fillStyle=theme.accent;ctx.font='600 30px ElfSignature, cursive';ctx.fillText(copy.signature||copy.santa,margin,signY,width-margin*2);
